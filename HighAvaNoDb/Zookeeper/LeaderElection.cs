...
namespace HighAvaNoDb.Zookeeper
{
	...
	public class LeaderElection : ProtocolSupport {
		...
		public bool RunForLeader() {
			...
			var names = Zookeeper.GetChildren(groupPath, false);
			...
			node = new ZNodeName(id);

			names = Zookeeper.GetChildren(groupPath, false);
			var sortedNames = new SortedSet<ZNodeName>(node as IComparer<ZNodeName>);
			foreach (var name in names) {
				sortedNames.Add(new ZNodeName(name));
			}

			var priors = sortedNames.HeadSet(node);
			...
			var penultimatePath = groupPath.Combine(penultimate.Name);
			if (Zookeeper.Exists(penultimatePath, new LeaderWatcher(Zookeeper, this, penultimatePath, watcher, groupPath, id)) == null) {
				IsOwner = true;
				watcher.TakeLeadership();
				return true;
			}
		...
		public override string ToString() {
			return string.Format("IdName: {0}, IsOwner: {1}", node, IsOwner);
		}
	}
}